...
describe('Create folder', function () {
  ...
  it('Create folder child', function () {
    ...
    expect(rowCountBeforeCreation + 1, 'Number of rows hasn\'t changed after creating new folder').equal(rowCountAfterCreation);
  });
  it('Should change name', function () {
    const newName = generateRandmString();
    const childFolderBeforeEdit = foldersPage.getFolderFromTree(foldersPage.getFolderRowNumByName(nameFolder), 1);
    childFolderBeforeEdit.editFolderChild(newName, null);
    const childFolderAfterEdit = foldersPage.getFolderFromTree(foldersPage.getFolderRowNumByName(nameFolder), 1);
    expect(childFolderAfterEdit.nameTree, 'Folder name has not changed').eq(newName);
  });
  it('Should change description', function () {
    const newDescription = generateRandmString();
    const childFolder = foldersPage.getFolderFromTree(foldersPage.getFolderRowNumByName(nameFolder), 1);
    childFolder.editFolderChild(null, newDescription);
    const childFolderDescription = childFolder.getDescription();
    expect(childFolderDescription, 'Folder description has not changed', newDescription);
  });
  it('Should not change first name and description if cancel was clicked', function () {
    ...
    const childFolderBeforeEdit = foldersPage.getFolderFromTree(foldersPage.getFolderRowNumByName(nameFolder), 1);
    const childFolderBeforeEditDescription = childFolderBeforeEdit.getDescription();
    ...
    const childFolderAfterEdit = foldersPage.getFolderFromTree(foldersPage.getFolderRowNumByName(nameFolder), 1);
    const childFolderAfterEditDescription = childFolderAfterEdit.getDescription();
    ...
    expect(childFolderAfterEditDescription, 'Description has been changed').equal(childFolderBeforeEditDescription);
  });
