...
describe('Device users page should not add new device user', function () {
  ...
  it('with only first name', function () {
    ...
    expect(deviceUsersPage.saveCreateBtn.isEnabled(), 'Create button in modal window while creating new device user is active when only name is provided').equal(false);
  });
  it('with only last name', function () {
    ...
    expect(deviceUsersPage.saveCreateBtn.isEnabled(), 'Create button in modal window while creating new device user is active when only last name is provided').equal(false);
  });
  it('without first and last names', function () {
    deviceUsersPage.newDeviceUserBtn.click();
    browser.pause(4000);
    expect(deviceUsersPage.saveCreateBtn.isEnabled(), 'Create button in modal window while creating new device user is active when both first name and last name are not provided').equal(false);
  });
