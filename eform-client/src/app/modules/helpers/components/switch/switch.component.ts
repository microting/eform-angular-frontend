...
@Component({
  selector: 'eform-switch',
  template: `
    <div #main class="bootstrap-switch bootstrap-switch-wrapper bootstrap-switch-animate bootstrap-switch-off"
         [ngClass]="_sizeClass"
         [class.bootstrap-switch-disabled]="(_isDisabledON && status == true) || (_isDisabledOFF && status == false)"
         (click)="toggleStatus()">
      <div #container class="bootstrap-switch-container"
           [@statusChange]="_statusStr">
      <span #on class="bootstrap-switch-handle-on"
            [ngClass]="_onColor"
            [ngStyle]="{'min-width': _minWidth +'px'}">{{onText}}</span>
...
export class SwitchComponent implements OnChanges, AfterViewInit {
  ...
  @Input() size = 'normal';
  @Input() isDisabledON = false;
  @Input() isDisabledOFF = false;
  ...
  toggleStatus(): void {
    if ((!this.isDisabledOFF && this.status === false) || (!this.isDisabledON && this.status === true)) {
    ...
    for (const propName in changes) {
      ...
      switch (propName) {
        ...
        case 'size':
          ...
          break;
        case 'isDisabledON':
          this._isDisabledON = this.isDisabledON;
          break;
        case 'isDisabledOFF':
          this._isDisabledOFF = this.isDisabledOFF;
      }
