...
import {
  ...
  OperationDataResult,
  OperationResult, PageSettingsModel
...
export class UserSettingsService extends BaseService {
  ...
  getLocalPageSettings(localStorageItemName: string, pageName: string): ApplicationPageModel {
    ...
    if (!localPagesSettings) {
      localStorage.setItem(localStorageItemName, JSON.stringify(ApplicationPagesSettings));
  ...
  updateUserSettings(model: UserSettingsModel): Observable<OperationResult> {
    return this.post<UserSettingsModel>(UserSettingsMethods.UserSettings, model);
  }
}
